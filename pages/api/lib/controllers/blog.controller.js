...
// bloglike count
const blogLikeCount = catchAsync(async (req, res) => {
  const blogId = req.params.blogId;
  const userId = new ObjectId(req.user);
  try {
    const data = await blogService.blogLikeCount(blogId, userId);
    res
      .status(httpStatus.OK)
      .json({ code: httpStatus.OK, message: "success", data: data });
  } catch (error) {
...
// get popular blog tags based on blogreadcount
const getPopularTags = catchAsync(async (req, res) => {
  try {
    ...
    console.log(
      `Exception :: CMS getPopularTags -> getPopularTags -> ${error.message}`
    );
    res.status(httpStatus.INTERNAL_SERVER_ERROR).json({ message: "error" });
  }
});
...
const getBlogBasedOnTitle = catchAsync(async (req, res) => {
  try {
    const data = await blogService.getBlogBasedOnTitle(req.query);
    res
      .status(httpStatus.OK)
      .json({ code: httpStatus.OK, message: "success", data: data });
  } catch (error) {
    console.log(
      `Exception :: Blog getBlogBasedOnTitle -> getBlogBasedOnTitle -> ${error.message}`
    );
    res.status(httpStatus.INTERNAL_SERVER_ERROR).json({ message: "error" });
  }
});

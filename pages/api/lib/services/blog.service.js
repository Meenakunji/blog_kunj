const { BlogContent, BlogLists, VisitedBlog, LikedBlog } = require("../models");
...
// search API
const getBlogBasedOnTitle = async (req) => {
  ...
  try {
    const regexQueries = searchWords.map((word) => ({
      blogTitle: { $regex: word, $options: "i" },
    }));

    const result = await BlogContent.find({ $or: regexQueries }).exec();

    return result;
  } catch (error) {
    console.error(
      "Error searching for blogs by title getBlogBasedOnTitle",
      error
    );
    throw error;
  }

...
export default function TemporaryDrawer() {
  ...
  const [replyMessageList, setReplyMessageList] = useState([]);
  ...
  const handleCreateBlogComment = (CommentType) => {
    let requestBodyCommentMsg;
    if (CommentType === "comment") {
      requestBodyCommentMsg = {
  ...
  // handle reply message list
  const handleReplyMessageModel = (replyMessage) => {
    setSelectedCommentId(replyMessage?.parentCommentId);

    // Check if 'replies' array contains non-empty data
    const filteredReplies = replyMessage?.replies?.filter(
      (reply) => reply && Object.keys(reply).length > 0
    );
    setHideReplies(!hideReplies);
    setReplyMessageList(filteredReplies);
  };
      ...
      <Box sx={style.commentChatBox}>
        ...
        <Box sx={style.bottomSection}>
          <Box sx={style.rightBottom}>
            ...
            <Button sx={style.respond} onClick={() => handleCreateBlogComment("comment")}>
              Respond
            </Button>
          </Box>
        </Box>
            ...
            <Box key={index}>
              <Box sx={style.replySection}>
                <Box sx={style.replyLeftProfile}>
                  <Image
                    src={item?.parentMessageUser?.[0]?.profilePic}
              ...
              <Box sx={style.replySectionComment}>
                ...
                <Box sx={style.replySectionBottom}>
                  ...
                  <Box
                    sx={style.replyRight}
                    onClick={() => handleReplyModel(item?.parentCommentId)}
                  >
                    <Typography variant="h5">Reply</Typography>
                  </Box>
                </Box>
                {selectedCommentId === item.parentCommentId && replyModel && (
                  <Box sx={style.commentChatBox}>
                    <Box sx={style.profileComment}>
                      <img
                    ...
                    <Box sx={style.bottomSection}>
                      <Box sx={style.rightBottom}>
                        <Button onClick={() => setReplyModel(false)}>Cancel</Button>
                        <Button sx={style.respond} onClick={() => handleCreateBlogComment("reply")}>
                          Respond
                        </Button>
                      </Box>
                    </Box>
                ...
                {/* show reply message details */}
                {selectedCommentId === item.parentCommentId &&
                  hideReplies &&
                  replyMessageList?.length > 0 &&
                  replyMessageList?.map((item, index) => {
                    return (
